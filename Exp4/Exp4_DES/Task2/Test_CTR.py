...
def validate_blocks_CTR(blocks, key1, key2, key3, initial_counter, is_encrypt=True, log_file=None):
    """验证CTR模式下的每个数据块（加密/解密），支持日志输出"""
    block_size = 8
    ...
    for idx, block_info in enumerate(blocks, 1):
        ...
        text_in_hex = block_info['TextIn'].replace(" ", "")
        text_out_hex = block_info['TextOut'].replace(" ", "")
        
        input_counter = int(input_block_hex, 16)
        output_block_int = int(output_block_hex, 16)
        text_in_bytes = bytes.fromhex(text_in_hex)
        text_out_bytes = bytes.fromhex(text_out_hex)
        
        key_stream_int = tdes_encrypt_block(counter, key1, key2, key3)
        ...
        log_line += (
            f"{'加密' if is_encrypt else '解密'}块: {text_in_hex.upper()}\n"
            f"块输入: {input_counter:016X}\n"
        ...
        counter_pass = (counter == input_counter)
        key_stream_pass = (key_stream_int == output_block_int)
        xor_pass = (xor_result_bytes == text_out_bytes)
        block_pass = counter_pass and key_stream_pass and xor_pass
...
def run_test_cases_CTR(test_cases, log_file=None):
    """运行CTR模式的所有测试用例，支持日志输出"""
    for case in test_cases:
        ...
        initial_counter = int(case['InitialCounterValue'].replace(" ", ""), 16)
        ...
        if "Encryption" in case['场景']:
            ...
            ciphertext = tdes_ctr_process(plaintext, key1, key2, key3, initial_counter)
            ...
            validate_blocks_CTR(
                case['Blocks'], key1, key2, key3, initial_counter,
            ...
            plaintext = tdes_ctr_process(ciphertext, key1, key2, key3, initial_counter)
...
if __name__ == "__main__":
    ...
    log_path = os.path.join(log_dir, "Test_CTR.txt")
    
    with open('../data_task2/_TDES_CTR.json', 'r', encoding='utf-8') as f:
        raw_data = json.load(f)
        std_data = json_data_process(raw_data)
        
    with open(log_path, 'w', encoding='utf-8') as log_file:
        run_test_cases_CTR(std_data, log_file=log_file)
